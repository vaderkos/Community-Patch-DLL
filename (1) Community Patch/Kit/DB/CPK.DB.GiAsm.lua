...
---
--- -- Gets row column value without throwing if column is not defined
--- maybe_nil = Gi.Policies.POLICY_LIBERTY('I_DO_NOT_EXIST')
---
--- -- Gets multiple row column values without throwing if column is not defined
...
--- This is a safe, non-throwing accessor.
---
...
--- @return number | string | boolean | nil
local gi_row_get_val = function(tbl_meta, row, key)
	...
	return gi_row_get_val_unsafe(tbl_meta, base, col_abi)
end
...
local gi_row_set_val = function()
	local p1 = 'GameInfo raw row is readonly!'
	local p2 = 'For modifications make a copy using GameInfo[table_name][row_key]()'
	lua_error(p1 .. ' ' .. p2)
end
...
--- - No arguments: returns a shallow Lua table copy of the row
--- - With column names: returns selected column values
---
--- Returned values are decoded and safe to use.
---
--- @param tbl_meta GiTblMeta
--- @param row GiRowTmp
--- @param ... nil | string
--- @return ... Selected values or a full row copy
local gi_row_call = function(tbl_meta, row, ...)
	...
	if arg_size == 1 then
		return gi_row_get_val(tbl_meta, row, lua_select(1, ...) --[[@as string | number]])
	end
	...
	for i = 1, arg_size do
		local key = lua_select(i, ...)

		if key ~= nil then
			res[i] = gi_row_get_val(tbl_meta, row, key)
		end
	end
